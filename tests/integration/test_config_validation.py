...
class ConfigValidator:
    ...
    def validate_model_configs(self):
        ...
        self.test(
            ...
            lambda: self._check_model(
                ModelType.GPT_5,
                {"temperature": 0.5},  # Should be forced to 1.0
                expected_temp=1.0
            )
        )
    ...
    def _check_model(self, model_type, params, expected_temp=None, 
                     has_max_output=False, has_reasoning_effort=False, has_top_p=False):
...
""")
            ...
            self.test(
                "YAML loads with env vars set",
                lambda: self._check_yaml_load_with_env(config_file)
            )
...
""")
    ...
    def _check_yaml_load_with_env(self, config_file):
        ...
        # Check that an API key is loaded (from either TEST_API_KEY interpolation or OPENAI_API_KEY env)
...
""")
            ...
            self.test(
                ...
                lambda: self._check_inheritance(exp_file, base_file)
            )
    ...
    def validate_cli_commands(self):
        ...
        self.test(
            ...
            lambda: __import__("cosmos_coherence.config.cli")
        )
    ...
    def validate_constraints(self):
        ...
        self.test(
            "O3 rejects top_p",
            lambda: self._expect_error(
                lambda: ModelConfig(model_type=ModelType.O3, top_p=0.9),
                "does not support"
            )
        )
    ...
    def _check_complete_experiment(self, model_type, benchmark_type, strategy_type, 
                                  k_responses=None, reasoning_effort=None):
        ...
        config = ExperimentConfig(
            ...
            benchmark=BenchmarkConfig(
                ...
                dataset_path=f"data/{benchmark_type.value}"
            ),
    ...
    def display_results(self):
        ...
        if success_rate == 100:
            console.print(Panel.fit(
                ...
                f"[green]{self.passed_tests}/{self.total_tests} tests passed ({success_rate:.0f}%)[/green]",
                box=box.ROUNDED
            ))
        else:
            console.print(Panel.fit(
                f"[bold yellow]⚠ VALIDATION INCOMPLETE[/bold yellow]\n"
                f"[yellow]{self.passed_tests}/{self.total_tests} tests passed ({success_rate:.0f}%)[/yellow]\n"
